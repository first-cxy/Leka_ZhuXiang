...
public class BTCommunicator extends Thread {
	...
	public static final int LIGHT_ON = 1040;
	public static final int GET_VALUE = 1041;
	public static final int LIGHT_OFF = 1042;

	public static final int NO_DELAY = 0;

	public static final int EV3_MOTOR_A = 1;
	public static final int EV3_MOTOR_B = 2;
	public static final int EV3_MOTOR_C = 4;
	public static final int EV3_MOTOR_D = 8;
	
	public static final int EV3_BEEP = 3051;
	public static final int EV3_STARTPROGRAM = 3052;
	public static final int EV3_INFOPROGRAM = 3053;
	public static final int EV3_MOTOR = 3054;
	...
	public BTCommunicator(BTConnectable myOwner, Handler uiHandler,
	...
	public void createNXTconnection() throws IOException {
		try {
			...
			//ImprovedBluetoothDevice nxtDevice=new ImprovedBluetoothDevice(mMACaddress);
			if (nxtDevice == null) {
				if (uiHandler == null)
...
/*
			//createInsecureRfcommSocket  createRfcommSocket
			Method mMethod1 = nxtDevice.getClass().getMethod(
					"createRfcommSocket", new Class[] { int.class });
			Method mMethod2 = nxtDevice.getClass().getMethod(
			...
			for(int port = 1; port < 31; port++) 
			{
			  try
			  {
			         BluetoothSocket s = (BluetoothSocket) mMethod1.invoke(
								nxtDevice, Integer.valueOf(port));
			           s.connect();
			           
			           nxtBTSocketTemporary = s;
			    } catch (Exception ex) 
			    {
			    	nxtBTSocketTemporary = null;
			      try 
			      {
			         BluetoothSocket s = (BluetoothSocket) mMethod2.invoke(
								nxtDevice, Integer.valueOf(port));
			           s.connect();
	...
	private void doEV3Beep(int frequency, int duration) {
		byte[] message = LCPMessage.getEV3BeepMessage(frequency, duration);
		sendMessageAndState(message);
		waitSomeTime(20);
	}

	private void doEV3StartProgram(int pos, String path, String name) {
		byte[] message = LCPMessage.getEV3StartProgramMessage(pos, path, name);
		sendMessageAndState(message);
		waitSomeTime(20);
	}
	
	private void doEV3InfoProgram()
	{
		byte[] message = LCPMessage.getEV3InfoProgramMessage();
		sendMessageAndState(message);
		waitSomeTime(20);
	}
	...
	private void changeEV3MotorSpeed(int motor, int speed) {
		if (speed > 100)
			speed = 100;
		...
		byte[] message = LCPMessage.getEV3MotorMessage(motor, speed);
		sendMessageAndState(message);
	}
	...
	final Handler myHandler = new Handler() {
		...
		public void handleMessage(Message myMessage) {
			...
			case DISCONNECT:
				...
				changeEV3MotorSpeed(EV3_MOTOR_A,0);
				changeEV3MotorSpeed(EV3_MOTOR_B,0);
				changeEV3MotorSpeed(EV3_MOTOR_C,0);
				changeEV3MotorSpeed(EV3_MOTOR_D,0);
				waitSomeTime(500);
				try {
			...
			case EV3_BEEP:
				doEV3Beep(myMessage.getData().getInt("value1"), myMessage
						.getData().getInt("value2"));
				break;
			case EV3_STARTPROGRAM:
				doEV3StartProgram(myMessage.getData().getInt("pos"), myMessage
						.getData().getString("path"), myMessage.getData()
						.getString("name"));
				break;
			case EV3_INFOPROGRAM:
				doEV3InfoProgram();
				break;
			case EV3_MOTOR:
				changeEV3MotorSpeed(myMessage.getData().getInt("value1"), myMessage
						.getData().getInt("value2"));
				break;
		...
		private void getValue() {
			byte[] message = LCPMessage.getValueMessage();
			sendMessageAndState(message);
		}

		private void setLight(int i) {
			byte[] message = LCPMessage.setLightMessage(i);
			sendMessageAndState(message);
		}
	};
	
	public void sendDrLeMessage(byte[] message) throws IOException{
		if(nxtOutputStream!=null){
